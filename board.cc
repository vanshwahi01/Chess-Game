...
bool Board::move(const Coordinate& c1, const Coordinate& c2, Board& b) { // c1 is start, c2 is end
    ...
    if(temp->isLegal(c2, b)) {
        ...
        if(c1.x % 2 == 0) { // Replace the removed piece with the correct tile
            if(c1.y % 2 == 0) td->notify('_', c1.x, c1.y);
            else td->notify(' ', c1.x, c1.y);
        } else {
            if(c1.y % 2 == 0) td->notify(' ', c1.x, c1.y);
            else td->notify('_', c1.x, c1.y);
        }

        if(temp->getType() == "Pawn") {
            if(temp->getColour() == Colour::Black) {
                td->notify('p', c2.x, c2.y);
            } else td->notify('P', c2.x, c2.y);
        }

        if(temp->getType() == "Rook") {
            if(temp->getColour() == Colour::Black) {
                td->notify('r', c2.x, c2.y);
            } else td->notify('R', c2.x, c2.y);
        }

        if(temp->getType() == "Knight") {
            if(temp->getColour() == Colour::Black) {
                td->notify('n', c2.x, c2.y);
            } else td->notify('K', c2.x, c2.y);
        }

        if(temp->getType() == "Bishop") {
            if(temp->getColour() == Colour::Black) {
                td->notify('b', c2.x, c2.y);
            } else td->notify('B', c2.x, c2.y);
        }

        if(temp->getType() == "King") {
            if(temp->getColour() == Colour::Black) {
                td->notify('k', c2.x, c2.y);
            } else td->notify('K', c2.x, c2.y);
        }

        if(temp->getType() == "Queen") {
            if(temp->getColour() == Colour::Black) {
                td->notify('q', c2.x, c2.y);
            } else td->notify('Q', c2.x, c2.y);
        }
...
void Board::setUpNormalBoard() {
    // Black pieces
    board[0][0] = new Rook({0,0}, Colour::Black, "Rook");
    td->notify('r', 0, 0);
        board[1][0] = new Knight({1,0}, Colour::Black, "Knight");
    td->notify('n', 1, 0);
        board[2][0] = new Bishop({2,0}, Colour::Black, "Bishop");
    td->notify('b', 2, 0);
    board[3][0] = new King({3,0}, Colour::Black, "King");
    td->notify('k', 3, 0);
    board[4][0] = new Queen({4,0}, Colour::Black, "Queen");
    td->notify('q', 4, 0);
        board[5][0] = new Bishop({5,0}, Colour::Black, "Bishop");
    td->notify('b', 5, 0);
        board[6][0] = new Knight({6,0}, Colour::Black, "Knight");
    td->notify('n', 6, 0);
        board[7][0] = new Rook({7,0}, Colour::Black, "Rook");
    td->notify('r', 7, 0);
    for (int i = 0; i < 8; ++i) {
        board[i][1] = new Pawn({i,1}, Colour::Black, "Pawn"); // row of pawns
        td->notify('p', i, 1);
    }
    ...
    for(int i = 0; i < 8; ++i) {
        board[i][6] = new Pawn({i,6}, Colour::White, "Pawn"); // row of pawns
        td->notify('P', i, 6);
    }

    // White pieces
    board[0][7] = new Rook({0,7}, Colour::White, "Rook");
    td->notify('R', 0, 7);
    board[1][7] = new Knight({1,7}, Colour::White, "Knight");
    td->notify('N', 1, 7);
    board[2][7] = new Bishop({2,7}, Colour::White, "Bishop");
    td->notify('B', 2, 7);
    board[3][7] = new Queen({3,7}, Colour::White, "Queen");
    td->notify('K', 3, 7);
    board[4][7] = new King({4,7}, Colour::White, "King");
    td->notify('Q', 4, 7);
    board[5][7] = new Bishop({5,7}, Colour::White, "Bishop");
    td->notify('B', 5, 7);
    board[6][7] = new Knight({6,7}, Colour::White, "Knight");
    td->notify('N', 6, 7);
    board[7][7] = new Rook({7,7}, Colour::White, "Rook");
    td->notify('R', 7, 7);
