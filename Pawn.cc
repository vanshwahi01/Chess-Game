...
vector<Coordinate> Pawn::possibleMoves(Board& b, Piece& p, Coordinate& c) {
    ...
    if(c.x > 0 && c.y < 7 && b.isOccupied({c.x - 1, c.y + 1})) { // Check if left diagnol is occupied
        if(b.getPiece({c.x - 1, c.y + 1})->getColour() != p.getColour()) { // Check if it is opposing team's piece
            pMoves.push_back({c.x - 1, c.y + 1});
        }
    }
    // we changed c.x < board.length - 1 to c.x < 7
    if(c.x < 7 && c.y < 7 && b.isOccupied({c.x + 1, c.y + 1})) { // Check if right diagnol is occupied
        if(b.getPiece({c.x + 1, c.y + 1})->getColour() != p.getColour()) { // Check if it is opposing team's piece
            pMoves.push_back({c.x + 1, c.y + 1});
        }
    }

    if(p.getNumMoves() == 0 && !(b.isOccupied({c.x, c.y + 1})) && !(b.isOccupied({c.x, c.y + 2}))) {
